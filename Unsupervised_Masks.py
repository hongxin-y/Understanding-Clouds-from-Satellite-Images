...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, list_IDs, batch_size=32, shuffle=False, width=512, height=352, scale=1 / 128., sub=1., step_per_epoch = 500,
                 mode='train',
    ...
    def __len__(self):
        if self.step_per_epoch != None: return self.step_per_epoch
...
h = model.fit_generator(train_gen, epochs=20, verbose=2, validation_data=val_gen)
...
for k in range(1, 5):
    ...
    print('f1 = ', f1)
print('OVERALL: ', end='')
auc = np.round(roc_auc_score(train3[['d1', 'd2', 'd3', 'd4']].values.reshape((-1)),
                             train3[['o1', 'o2', 'o3', 'o4']].values.reshape((-1))), 3)
acc = np.round(accuracy_score(train3[['d1', 'd2', 'd3', 'd4']].values.reshape((-1)),
                              (train3[['o1', 'o2', 'o3', 'o4']].values > 0.5).astype(int).reshape((-1))), 3)
f1 = np.round(f1_score(train3[['d1', 'd2', 'd3', 'd4']].values.reshape((-1)),
                       (train3[['o1', 'o2', 'o3', 'o4']].values > 0.5).astype(int).reshape((-1))), 3)
