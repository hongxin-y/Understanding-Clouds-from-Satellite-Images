...
def read_data(file_path):
    ...
    data_df[['is_fish', 'is_flower', 'is_gravel', 'is_sugar']] = (
                data_df[['rle_fish', 'rle_flower', 'rle_gravel', 'rle_sugar']] != '').astype('int8')
    return data_df


class DataGenerator(keras.utils.Sequence):
    def __init__(self, id_list, batch_size=8, width=512, height=352, shuffle=False, mode="train", path=IMG_PATH,
                 flips=False):
...
def evaluation_class(model, test_df, threshold=0.5, mode="testing"):
    ...
    test_df.loc[:, 'pred_fish'], test_df.loc[:, 'pred_flower'], test_df.loc[:, 'pred_gravel'], test_df.loc[:,
                                                                                               'pred_sugar'] = 0, 0, 0, 0
    ...
    for k, label in enumerate(LABELS):
        ...
        log += "AUC = " + str(np.round(auc, 3)) + ", ACC = " + str(np.round(acc, 3)) + ", f1 = " + str(
            np.round(f1, 3)) + "\n"
    ...
    acc = accuracy_score(test_df[['is_fish', 'is_flower', 'is_gravel', 'is_sugar']].values.reshape(-1), \
                         (test_df[['pred_fish', 'pred_flower', 'pred_gravel', 'pred_sugar']].values > th).astype(
                             int).reshape((-1)))
    f1 = f1_score(test_df[['is_fish', 'is_flower', 'is_gravel', 'is_sugar']].values.reshape(-1), \
                  (test_df[['pred_fish', 'pred_flower', 'pred_gravel', 'pred_sugar']].values > th).astype(int).reshape(
                      (-1)))
    log += "AUC = " + str(np.round(auc, 3)) + ", ACC = " + str(np.round(acc, 3)) + ", f1 = " + str(
        np.round(f1, 3)) + "\n"
    return log
...
def evaluation_segmentation(test_df, thresholds=[0.5,0.65,0.6,0.65], mode="testing"):
    log = "Evaluation of " + mode + " data:\n"
    '''
    for k, label in enumerate(LABELS):
        test_df['score_' + label] = test_df.apply(
            lambda x: dice_coef(x["rle_" + label], x["pred_rle_" + label], x["pred_vec_" + label], thresholds[k]),
            axis=1)
    ...
    for k, label in enumerate(LABELS):
        dice_k = []
        for th in np.arange(0.1, 0.8, 0.05):
            test_df['score_' + label] = test_df.apply(
                lambda x: dice_coef(x["rle_" + label], x["pred_rle_" + label], x["pred_vec_" + label], th),
                axis=1)
            dice = test_df['score_' + label].mean()
            dice_k.append(np.around(dice,3))
            log += label + ": Kaggle Dice =" + str(np.around(dice, 3)) + " Threshold =" + str(np.around(th, 3)) + "\n"
        dice_th.append(dice_k)

    dice_th = np.array(dice_th)
    np.save("dice_th.npy", dice_th)
...
def generate_segmentation(cam, weights, test_df, th=0.3):
    test_df.loc[:, 'pred_rle_fish'], test_df.loc[:, 'pred_rle_flower'], test_df.loc[:, 'pred_rle_gravel'], test_df.loc[
                                                                                                           :,
                                                                                                           'pred_rle_sugar'] = "", "", "", ""
    test_df.loc[:, 'pred_vec_fish'], test_df.loc[:, 'pred_vec_flower'], test_df.loc[:, 'pred_vec_gravel'], test_df.loc[
                                                                                                           :,
                                                                                                           'pred_vec_sugar'] = 0, 0, 0, 0
...
def save_segmentation(cam, weights, num, path, thresholds=[0.8, 0.5, 0.7, 0.7]):
    ...
    for img_idx in test_df.index:
        ...
        for label_idx in [0, 1, 2, 3]:
            ...
            skimage.io.imsave(path + img_idx + "_pred_" + label + ".jpg",
                              (mask_pred > th[label_idx]).astype("uint8") * 100)
...
if __name__ == "__main__":
    ...
    # save figures
    # segmentation_ths = [0.3, 0.4, 0.2, 0.3]
    # save_segmentation(cam, weights, 25, "./", thresholds=segmentation_ths)
    # save_segmentation(cam, weights, 25, "./res_fig1/", thresholds=segmentation_ths)